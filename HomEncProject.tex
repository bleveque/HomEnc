...
The tool that helps solve the IMP in polynomial rings is called a \emph{Gr\"obner basis} for an ideal. We define two separate (but similar) notions of a Gr\"obner basis below. Note that the definitions below are given in the context of $\Zxy$, but they are applicable for polynomial rings in arbitrary numbers of variables over general rings.
...
Now, imagine an eavesdropper had access to a Gr\"obner basis for the ideal $(f,g)$ in our multivariate encryption scheme. FINISH FILLING IN DETAILS OF WHY KNOWING GB BREAKS SCHEME
